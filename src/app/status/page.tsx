...
export default async function StatusPage() {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6 px-6 py-16">
      <div>
        <h1 className="pixel-heading text-lg text-stone-900">Your applications</h1>
      ...
      {!applications || applications.length === 0 ? (
        <Card className="flex flex-col gap-4">
          <p className="text-sm text-stone-600">You haven&apos;t started an application yet.</p>
          <Link href="/apply">
        ...
        <div className="flex flex-col gap-4">
          {applications.map((app) => (
            <Card key={app.id} className="flex items-center justify-between gap-4">
              <div>
                <h2 className="font-medium text-stone-900">{TRACK_LABEL[app.track]}</h2>
          ...
          <Link href="/apply" className="text-sm text-bamboo-700 hover:underline">
